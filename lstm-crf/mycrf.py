...
class ConditionalRandomField(nn.Module):
    r"""
    条件随机场。提供 :meth:`forward` 以及 :meth:`viterbi_decode` 两个方法，分别用
    于 **训练** 与 **inference**。
    ...
    def __init__(self,
                 num_tags: int,
                 include_start_end_trans: bool = False,
    ...
    def _normalizer_likelihood(self, logits, mask):
        r"""Computes the (batch_size,) denominator term for the
        log-likelihood, which is the sum of the likelihoods across all
    ...
    def _gold_score(self, logits, tags, mask):
        ...
        mask = mask.eq(True)
        flip_mask = mask.eq(False)
        trans_score = self.trans_m[tags[:seq_len - 1],
                                   tags[1:]].masked_fill(flip_mask[1:, :], 0)
        # emit_score [L, B]
        emit_score = logits[seq_idx.view(-1, 1),
                            batch_idx.view(1, -1),
                            tags].masked_fill(flip_mask, 0)
        # score [L-1, B]
        score = trans_score + emit_score[:seq_len - 1, :]
        score = score.sum(0) + emit_score[-1].masked_fill(flip_mask[-1], 0)
        if self.include_start_end_trans:
            st_scores = self.start_scores.view(1, -1).repeat(batch_size,
                                                             1)[batch_idx,
                                                                tags[0]]
            last_idx = mask.long().sum(0) - 1
            ed_scores = self.end_scores.view(1, -1).repeat(batch_size,
                                                           1)[batch_idx,
                                                              tags[last_idx,
                                                                   batch_idx]]
    ...
    def forward(self, feats: 'torch.FloatTensor', tags: 'torch.LongTensor',
                mask: 'torch.ByteTensor') -> 'torch.FloatTensor':
        r"""
        用于计算 ``CRF`` 的前向 loss，返回值为一个形状为 ``[batch_size,]`` 的
        :class:`torch.FloatTensor`，可能需要 :func:`mean` 求得 loss 。

        :param feats: 特征矩阵，形状为 ``[batch_size, max_len, num_tags]``
        :param tags: 标签矩阵，形状为 ``[batch_size, max_len]``
        :param mask: 形状为 ``[batch_size, max_len]``，为 **0** 的位置认为是
            padding。
        :return: ``[batch_size,]``
        """
        ...
        return (all_path_score - gold_path_score).mean()
    ...
    def _viterbi_decode(self,
                       ...
                       unpad=False):
        r"""给定一个 **特征矩阵** 以及 **转移分数矩阵**，计算出最佳的路径以及对应的
        分数

        :param logits: 特征矩阵，形状为 ``[batch_size, max_len, num_tags]``
        :param mask: 标签矩阵，形状为 ``[batch_size, max_len]``，为 **0** 的位置
